...
export default function Login() {
    ...
    const onSubmit = data => {
        axios.get("http://localhost:4000/api/user/create/" + data.username + "/" + data.password + "/" + data.displaynae)
            .then((response) => {
    ...
    return (
        ...
        <div className="h-screen bg-gray-300 flex justify-center items-center">
            <form onSubmit={handleSubmit(onSubmit)} className="w-80 bg-white rounded px-4 py-8">
                ...
                {/*  */}
                <label htmlFor="displayname" className="mb-5">Display Name:</label><br></br>
                <input {...register("displayname", { required: true })} id="displayname" placeholder="displayname" className="border text-grey-darkest rounded-md mb-10 p-2" /><br />
